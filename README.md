...
- Dodge / Roll: add an Input System action named `Dodge` (or change `Input → Dodge Action`) and add an Animator trigger parameter named `Roll` (or change `Animation → Roll Trigger`). Create an Any State → Roll transition driven by that trigger, and exit back to locomotion after the clip. If your Animator still delays the roll (due to exit times), set `Animation → Roll State Name` to your roll state (e.g. `Roll` or `Base Layer.Roll`) to force-play it immediately.
